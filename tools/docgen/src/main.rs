...
// TODO: no example mode
// TODO: optional cli key insert
...
fn main() {
    ...
    for line in lines {
        ...
        if mode == DocMode::Cli {
            ...
            // out_buf = BufWriter::new(v);
            prepare_page(path_str, &data, &mut out_buf);
            // file_buf.write(out_buf);
        }
    }

    // if mode == DocMode::Cli {
    //     let mut file_buf = BufWriter::new(File::create(out_path).unwrap());
    //     file_buf.write(out_buf);
    // }
}
...
fn gen_table(lines: &mut Iter<String>, data: &Data) -> String {
    ...
    let svg1_bytes = gen_svg_file(&svg1, Column::Before, data);
    let svg2_bytes = gen_svg_file(&svg2, Column::After, data);
...
fn gen_svg_file(lines: &Vec<String>, col: Column, data: &Data) -> usize {
    let mut s = String::new();
    for l in lines {
        if l.starts_with("<svg") {
            let line = l.replace("<svg",
    ...
    // let doc = Document::from_data(s.as_bytes()).unwrap();
    // let svg_node = doc.root().child_by_tag_name(&TagName::Id(ElementId::Svg)).unwrap();
    // svg_node.set_attribute(AttributeId::Xmlns, "http://www.w3.org/2000/svg");
    // svg_node.set_attribute(AttributeId::XmlnsXlink, "http://www.w3.org/1999/xlink");
    // svg_node.set_attribute(AttributeId::Width, 200.0);
    // svg_node.set_attribute(AttributeId::Height, 100.0);

    // let mut wo = WriteOptions::default();
    // wo.indent = 2;

    // let mut out = Vec::new();
    // doc.write_with_opt(&wo, &mut out);
